...
int main(){
  ...
  ch = 0;
    ...
    if (ch > 7){
      for (i=0; i<8; i++){
        //printf(" ch%d = %d\n",i,ad_read(pd, fd, i));
        printf("%d",ad_read(pd, fd, i));
      }
    }
    //else if (ch >= 0) printf(" ch%d = %d\n",ch,ad_read(pd, fd, ch));
    else if (ch >= 0) printf("%d",ad_read(pd, fd, ch));
...
int ad_read(int pd, int fd, int ch){
// mcp3008 から A/D 変換値を読み出す関数
// バッファを用意して、そこに制御ビットを入れて書き込むと値がバッファに返る
// 以下、MCP3008 の仕様
// 送受信に必要なバッファサイズは 3 バイト、バッファの内容が書き換わるので注意
// 送信時：0 バイト目 0000 0001：ビット 0 はスタートビットで'1'、その他は'0'
// 1 バイト目 SCCC XXXX：S はシングルエンド指定'1'、CCC は入力チャンネル指定
// 2 バイト目 XXXX XXXX：なんでもよい
// 受診時：0 バイト目 ???? ????：不定、A/D 値とは無関係
// 1 バイト目 ???? ?0BB：?は不定、BB は上位 2 ビット（B9,B8）
// 2 バイト目 BBBB BBBB：下位 8 ビット（B7-B0）

  unsigned char wdata[3], rdata[3]; // SPI でやりとりするためのバッファ
  int val;
  wdata[0] = 0x01; // スタートビットになる
  wdata[1] = (0x80) | (ch << 4); // シングルエンド、チャンネルセット
  spi_xfer(pd, fd, wdata, rdata, 3); // SPI データの送受信
  val = ((rdata[1] & 0x03) << 8); // 上位 2 ビット分を取り込んでシフト
  val = val + rdata[2]; // 下位 8 ビット分を取り込んで加算
  return val;
}
